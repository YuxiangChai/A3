...
def task() -> str:
    return {
        "task": "Sort my 'Homework' list by starred recently in Google tasks.",
        "level": "easy",
        "category": "operation",
    }
...
def eval(
    ...
    sort_star_element = parser.get_element_bydic(
        {
            "text": "Starred recently",
            "resource-id": "com.google.android.apps.tasks:id/task_list_menu_item_text",
        }
    )

    sort_star_button = parser.find_clickable_parent(sort_star_element)

    # "[1,2][3,4]" -> [1,2,3,4]
    bounds_str = sort_star_button.attrib["bounds"].replace("][", ",")
